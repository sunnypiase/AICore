...
class TradingModelTrainer:
    ...
    def create_model(self, timesteps, features, isLstm = True):
        model = Sequential()
        if isLstm: 
            model.add(LSTM(32, return_sequences=True, input_shape=(timesteps, features)))
            ...
            model.add(Dense(64, input_shape=(self.trainData.shape[1],)))
            model.add(BatchNormalization())
    ...
    def simulate_trading(self, model: Sequential, timesteps: int) -> Trader:
        ...
        with tf.device('/GPU:0'):  # Specify the GPU device # type: ignore
    ...
    def update_learning_history(self, sharpe_ratio: float):
        self.model_learning_history.append({'sharpe_ratio': sharpe_ratio})
